...
| `bnk heal` | Detect and repair the cluster plumbing `up` and `bnk upgrade` also fix, on 2.3 or 2.4: Multus token watch, metrics-server add-on, TMM log stream, pod-manager cert mount and crash loop, cwc crash loop, dSSM probe overlay, controller EndpointSlice RBAC, controller IRSA, `TMM_K8S_ROUTES` (`-f` for the AWS-backed ones, `--kubeconfig`, `--only`, `--dry-run`, `-o json`); `doctor --backend k8s` shows the same detections |
