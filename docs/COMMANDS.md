...
| `bnk heal` | Repair the cluster plumbing `up` and `bnk upgrade` also fix, on 2.3 or 2.4: creates the metrics-server EKS add-on (with `-f`) so `metrics.k8s.io` serves CPU/memory to Forge; adds `--cleanup-config-on-exit=true` to the Multus DaemonSet so its kubeconfig follows the service-account token, rolls it when a pod hit `Multus ... Unauthorized` (`-f`, `--kubeconfig`, `--dry-run`, `-o json`) |
