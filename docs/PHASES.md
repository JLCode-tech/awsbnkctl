...
- **`spk-vlan-gateway-class`** (`Phase23bSPKVlanGatewayClass`): Applies the BNK 2.4 `Infra` CR (external/internal VLANs with pinned self IPs, the VIP listener pool) and the GatewayClass. Waits for the cne-controller to be available first, because its pod serves the F5 validating webhook, then for Infra `Programmed=True`. The 2.3.x F5SPKVlan CRs are gone; `down` still deletes them on older clusters.
