...
- **`spk-vlan-gateway-class`** (`Phase23bSPKVlanGatewayClass`): Applies the GatewayClass, waits for it to be Accepted, then applies the BNK 2.4 `Infra` CR (external/internal VLANs with pinned self IPs, the VIP listener pool) and waits for `Programmed=True`. The controller only reconciles Infra once a GatewayClass exists. Waits for the cne-controller to be available first, because its pod serves the F5 validating webhook. The 2.3.x F5SPKVlan CRs are gone; `down` still deletes them on older clusters.
