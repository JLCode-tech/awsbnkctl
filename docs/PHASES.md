...
- **`secondary-enis`** (`Phase17SecondaryENIs`): Creates secondary ENIs for the data plane (internal/external) attached directly to the worker node and puts each TMM self-IP pool (`<subnet>.240`–`.243`) on its ENI as secondary IPs, because AWS only delivers traffic for addresses the ENI owns.
...
- **`spk-vlan-gateway-class`** (`Phase23bSPKVlanGatewayClass`): Applies the GatewayClass, waits for it to be Accepted, then applies the BNK 2.4 `Infra` CR (external/internal VLANs with 4-address self-IP pools, the VIP listener pool), waits for `Programmed=True`, then records the self IP the F5 IPAM controller allocated as `TMM_EXT_SELFIP` / `TMM_INT_SELFIP`. The pools are /30s because the F5 IPAM controller splits every pool into per-device blocks and rejects fewer than 3. The controller only reconciles Infra once a GatewayClass exists. Waits for the cne-controller to be available first, because its pod serves the F5 validating webhook. The 2.3.x F5SPKVlan CRs are gone; `down` still deletes them on older clusters.
