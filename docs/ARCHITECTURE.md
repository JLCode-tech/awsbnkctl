...
BNK 2.3.2 and 2.3.3 release notes list for host Kubernetes, and the one every
...
`awsbnkctl` always targets the **newest 2.3.x release manifest** F5 has published
on `repo.f5.com` — `manifest.DefaultManifestVersion`, currently
`2.3.3-3.2598.3-0.0.509`. Every example and every scenario is written against
that default. Older 2.3.x builds stay deployable: set `bnk.manifestVersion` in
`cluster.yaml` and the rest of the stack follows.

| Release manifest | FLO chart it ships with | Status |
| --- | --- | --- |
| `2.3.3-3.2598.3-0.0.509` | `v2.21.13-0.0.64` | default |
...
What was checked against the 2.3.3 charts (2026-09-10): the `CNEInstance` CRD
spec has the same field set in 2.3.0 and 2.3.3, so the embedded template applies
to every build; the F5 CRD API versions the scenarios and examples use
(`k8s.f5net.com/v1` for F5BnkGateway and F5SPKVlan, `k8s.f5net.com/v3` for
F5SPKEgress, `k8s.f5net.com/v1alpha1` for RoutingTemplate / GlobalRoutingConfig,
`gateway.k8s.f5net.com/v1alpha1` for BNKNetPolicy / BNKSecPolicy) are all served
and validated by the 2.3.3 `f5ingress` webhook; and the `format: int32` +
`maximum: 4294967295` defect that blocks Kubernetes 1.36 is still present in the
2.3.3 CRD charts, so the 1.36 warning stands.
