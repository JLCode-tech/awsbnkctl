...
create by hand, how to apply the routing CRs, how to verify, and how to tear it
down without stranding the cluster.

## What `bnk.bgp: true` does, and does not do

The TMM pod already runs the routing container (ZebOS, `f5-tmm-routing`) on
every cluster awsbnkctl builds — the embedded CNEInstance enables
`dynamicRouting`. What stops a peer from talking to it is two closed doors, and
`bnk.bgp` opens both:
...
- These values from `.awsbnkctl/<cluster>/state.env`:
  ...
  TMM_EXT_SELFIP=$(grep ^TMM_EXT_SELFIP= $STATE | cut -d= -f2)   # 10.0.10.240 in the examples
  ```
...
## 4. Create the peer pointing at TMM's external SelfIP
...
## 5. Apply the routing CRs

Edit the example's `bgp-route-server.yaml`: replace every `10.0.10.31` with
`$RSE_IP`. Then:

```bash
export KUBECONFIG=.awsbnkctl/<cluster>/kubeconfig
kubectl apply -f examples/<example>/bgp-route-server.yaml
kubectl -n f5-cne-system get routingtemplate,globalroutingconfig
kubectl -n f5-cne-system get globalroutingconfig <cluster>-bgp-config \
  -o jsonpath='{.status.conditions[?(@.type=="Programmed")]}'; echo
```

`Programmed=True` means the controller pushed the stanza into the routing
container. If it reads `Failed` with a gRPC error, recreate the TMM pod once
(`kubectl -n f5-cne-system delete pod -l app=f5-tmm`) and re-apply; the routing
container's gRPC server occasionally is not up when the first push lands.

## 6. Verify

```bash
# AWS side: session and BFD up, VIP learned
aws ec2 describe-route-server-peers --route-server-endpoint-id $RSE_ID \
  --query 'RouteServerPeers[].[RouteServerPeerId,State,BgpStatus.Status]' --output text
aws ec2 get-route-server-routing-database --route-server-id $RS_ID
aws ec2 describe-route-tables --route-table-ids $PUBLIC_RTB \
  --query 'RouteTables[0].Routes[?Origin==`EnableVgwRoutePropagation` || contains(DestinationCidrBlock, `/32`)]'

# TMM side: neighbor Established
...
kubectl -n f5-cne-system exec $TMM -c f5-tmm-routing -- imish -e 'show ip bgp neighbors'
```

Expect a `/32` for the Gateway VIP (10.0.10.100 in the examples) with the TMM
external ENI as its target in every propagated route table.

## Teardown (before `awsbnkctl down`)

```bash
kubectl delete -f examples/<example>/bgp-route-server.yaml
for p in $(aws ec2 describe-route-server-peers --route-server-endpoint-id $RSE_ID \
    --query 'RouteServerPeers[].RouteServerPeerId' --output text); do
...
- [BNK: set up dynamic routing with BGP](https://clouddocs.f5.com/bigip-next-for-kubernetes/latest/how-tos/spk-zebos-config.html)
- [F5SPKVLAN `allowed_services`](https://clouddocs.f5.com/bigip-next-for-kubernetes/2.0.0-LA/spk-vlan-crd.html)
