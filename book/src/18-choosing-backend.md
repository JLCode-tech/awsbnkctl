...
Every tool has a default backend baked into `awsbnkctl`. Workspace config (`exec:` block) can override the default per workspace; `--backend` overrides for a single invocation.

| Tool | Default | Resolved by |
|---|---|---|
| `aws` | `local` | `internal/cli/cluster.go::resolveBackendSpecWith("aws", flagOverride)` |
...
- **`aws` defaults to `local`** because most users have it on PATH or are happy installing it. The compliance + firewall scenarios where `ssh` or `docker` are better are the minority of calls.
...
- **`terraform` defaults to `local`** because the terraform-exec local path is the established workflow. State handling is simplest there. Frozen-version CI runs use `--backend docker`; non-local network-locality use cases (cluster-side, SSH-bastion-side) are deferred to a future release pending a state-handling design — see [PRD 03 §"State concerns"](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/prd/03-EXECUTION-BACKENDS.md#terraform).

To change a default per workspace, edit `~/.awsbnkctl/<workspace>/config.yaml`:

```yaml
exec:
  iperf3:    { backend: k8s }      # already the default; shown for clarity
  aws:       { backend: ssh:bastion }
...
|---|---|---|---|---|
| `aws` | yes (default) | yes (frozen image) | yes (long-lived ops pod) | yes |
...
- **deferred to v1.x** — a real design constraint, not a gap; see the cell text and [PRD 03 §"State concerns"](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/prd/03-EXECUTION-BACKENDS.md#terraform).
- **internalised** — `awsbnkctl` performs the operation via its own embedded library, not by shelling out; no backend selection applies.
...
- **`terraform` over `k8s` and `ssh` is deferred to v1.x**. The state file is sensitive (admin tokens, generated TLS keys, license bundles); moving it into a Kubernetes Secret or scp'ing it pre/post-run requires a state-handling design that hasn't shipped yet. [PRD 03 §"State concerns"](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/prd/03-EXECUTION-BACKENDS.md#terraform) lays out the considerations; the roadmap entry lives in [`docs/PLAN.md`](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/PLAN.md) §"What's deliberately deferred to post-v1.0".

Passing an unsupported `(tool, backend)` pair errors at the CLI layer before the backend is invoked:

```
$ awsbnkctl test throughput --backend docker
...
# equivalent to:
awsbnkctl test throughput --backend k8s
```

The k8s backend deploys a server-side Deployment + LoadBalancer Service in `awsbnkctl-test`, runs the iperf3 client as a one-shot Job in the same namespace, collects the JSON output from the client pod's logs, and tears down both. The bandwidth number reflects the cluster fabric.

If you instead want to measure your laptop's uplink to the cluster:

```bash
awsbnkctl test throughput --backend local --endpoint <cluster-LB-ip>:5201
```
...
The multi-vantage probe ships at v1.0 via `awsbnkctl test dns --gslb-compare`:
...
### "I need to run `aws` from a customer-firewalled office"

Use `--backend ssh:<bastion>`. Your customer's network policy lets the corporate jumphost reach `*`*.amazonaws.com` but blocks your laptop. The SSH backend ships your kubeconfig to the bastion (single file, mode `0600`, removed via `trap` on session exit), runs `aws` there, streams the output back:

```bash
awsbnkctl exec --backend ssh:bastion ks cluster ls
```

If `aws` isn't installed on the bastion, you'll get a clear error:

```
error: tool `aws` not found on ssh target bastion; re-run with --bootstrap to install
       via apt-get, or pre-install on the target manually
```

Re-run with `--bootstrap` if you want `awsbnkctl` to `sudo apt-get install -y aws-cli` on the bastion. The opt-in default reflects "we don't surprise users with `sudo apt-get` on a remote they didn't expect mutation on" — see [Chapter 17 §"SSH backend"](./17-execution-backends.md#ssh-backend) for the bootstrap mechanics.

### "I'm in CI and want a frozen toolchain version"

Use `--backend docker`. The vendored images are tagged in lock-step with `awsbnkctl` releases — `ghcr.io/JLCode-tech/awsbnkctl-tools-aws:v1.0.0` is the exact same `aws` binary every CI run sees, regardless of when the runner image was built or what `apt-get` happens to ship that day:

```bash
awsbnkctl exec --backend docker iam oauth-tokens
awsbnkctl up --backend docker     # terraform inside hashicorp/terraform:<v>
```

For CI specifically, also pin `aws.credentials_source: env` in workspace config so the API key resolution is unambiguous (no keychain fallback to confuse a non-interactive runner) — see [Chapter 14 §"Pinning a single source"](./14-credentials-resolver.md#pinning-a-single-source).
...
Use `--backend k8s` with the long-lived ops pod. Once `awsbnkctl ops install` has run, `--backend k8s` for `aws` (or any future tool) routes through `kubectl exec -n awsbnkctl-ops ops -- <argv>`. The pod stays alive between invocations, so the second and subsequent commands skip pod-startup latency.

```bash
awsbnkctl ops install
awsbnkctl exec --backend k8s iam oauth-tokens
awsbnkctl exec --backend k8s ks cluster ls
awsbnkctl exec --backend k8s account list
```
...
Use `local` or `ssh:<target>`. The `k8s` backend prereq is a working kubeconfig pointing at a running cluster; before `awsbnkctl up` has succeeded, that doesn't exist. For pre-cluster aws + terraform calls (account inspection, IAM tinkering, the cluster-create itself), `local` and `ssh:bastion` are the only two paths.

## When *not* to use a backend

Common foot-guns, in rough order of how often they come up:

### `--backend k8s` without `awsbnkctl ops install`

The ops pod must exist before the k8s backend can route `aws` calls through it. First-time use:

```bash
awsbnkctl ops install         # one-time setup per cluster
awsbnkctl exec --backend k8s ks cluster ls
```
...
```
error: ops pod not found in awsbnkctl-ops namespace; run `awsbnkctl ops install` first
```
...
If `aws` (or `iperf3`) isn't installed on the target, the SSH backend won't silently `sudo apt-get` for you — `--bootstrap` is opt-in. The first call on a fresh target tells you exactly what's needed:
...
```
error: auto-install only supports Ubuntu. Pre-install `aws-cli` on the target
       (RHEL: `yum install aws-cli`)
```
...
Deferred to v1.x. The `terraform` tool's k8s + ssh backends require a state-handling design that hasn't shipped — moving the state file into a Kubernetes Secret or scp'ing it pre/post-run is fiddly enough to be a feature in its own right ([PRD 03 §"State concerns"](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/prd/03-EXECUTION-BACKENDS.md#terraform); roadmap in [`docs/PLAN.md`](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/PLAN.md) §"What's deliberately deferred to post-v1.0"). For now, `terraform` supports `local` and `docker` only. If the network-locality use case (running `terraform` from a customer VPC for IP-egress reasons) is blocking, file an issue.

### Mixing `--on` and `--backend ssh:<target>`

`--on <target>` is the [Chapter 16](./16-on-flag-ssh-jumphosts.md) lightweight remote-exec — it runs the *passthrough* shape (`exec`, `shell`, `kubectl`, `oc`, `aws`) on the target by literally re-running the command via SSH. `--backend ssh:<target>` is the heavier-duty form — it routes through the `Backend` interface, which means file materialisation, env propagation hardening, opt-in apt-bootstrap, and the redactor are all wired in.

You generally want one or the other, not both. The supported precedence is "`--backend ssh:<target>` wins"; passing both flags on the same invocation surfaces a warning. If you're calling `awsbnkctl exec -- …`, prefer `--backend ssh:<target>` for the same target — you get the better cred-handling story automatically.

## Workspace config + `--backend` flag interaction

Recap of [Chapter 12 §"`exec:`"](./12-workspace-config.md):

The flag wins. If `~/.awsbnkctl/<ws>/config.yaml` says:
...
…and you run `awsbnkctl test throughput --backend local`, the `local` backend runs. The flag is the **per-invocation override**; the workspace config is the **per-workspace default**.

If neither is set, the per-tool default from the previous section applies (iperf3 → k8s, aws → local, terraform → local). The resolution order is exact:
...
| `aws` from a customer-firewalled office | `ssh:bastion` | with `--bootstrap` if first call on fresh Ubuntu |
| Frozen-version CI for any tool | `docker` | image tag matches `awsbnkctl` release |
| Cluster-side ad-hoc `aws` debugging | `k8s` | requires `awsbnkctl ops install` first |
| Pre-cluster aws / terraform | `local` or `ssh` | `k8s` requires a working cluster |
| `terraform up` on a clean dev machine | `local` (default) or `docker` | k8s + ssh deferred |
| Air-gapped: laptop can't reach AWS, bastion can | `ssh:bastion` | with kubeconfig propagation |
| Just learning the tool | `local` | simplest mental model |

## Worked example: bare-metal + jumphost office workflow

End-to-end Part V scenario: you're an F5 SE running a customer POC from a corporate-firewalled office. The laptop can't reach `*`*.amazonaws.com` directly (the office proxy blocks it) but a customer-provisioned Ubuntu jumphost at `10.20.30.40` can. The jumphost was already auto-discovered by an earlier `awsbnkctl up` against this customer's account, so `targets list` shows it. You need to: install the in-cluster ops pod, run `aws` from the bastion, and run a throughput test from inside the cluster — all without installing tools locally.

```bash
# 1. Verify jumphost is registered + reachable
$ awsbnkctl targets list -w customer
NAME       HOST          KEY_SOURCE         STATUS
jumphost   10.20.30.40   workspace/state    reachable

# 2. Run aws from the jumphost (Sprint 1 --on flag, lightweight)
$ awsbnkctl aws --on jumphost ks cluster ls
OK
...
$ awsbnkctl test throughput --backend ssh:jumphost --mode north-south
✓ throughput: 936 Mbits/sec  (jumphost → cluster LB; customer's WAN)

# 8. Persist the per-tool routing in workspace config (one-time)
$ cat >> ~/.awsbnkctl/customer/config.yaml <<'YAML'
exec:
  aws:       { backend: ssh:jumphost }
  iperf3:    { backend: k8s }
  terraform: { backend: local }
YAML
# Subsequent runs skip the --backend flag — every aws call routes via
# the jumphost automatically; every iperf3 runs in-cluster.
```

The point of this walkthrough: with no tools installed locally beyond `awsbnkctl` itself, you've reached the AWS control plane from the customer's bastion (compliance-correct egress), exercised the cluster fabric for throughput, and persisted the routing per workspace. The same laptop with the same `awsbnkctl` binary handles a different customer's POC by pointing at a different workspace; nothing on the laptop is workspace-specific.
...
- [PRD 03 — pluggable execution backends](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/prd/03-EXECUTION-BACKENDS.md) — the design spec.
