...
>
> Workspaces created against v1.0.x that have cluster modules and trial modules in the same `terraform.tfstate` (the legacy single-state shape) keep working — `awsbnkctl up` and `down` continue to operate against them in-place, byte-for-byte the way they did in v1.0. See [§ Legacy single-state workspaces](#legacy-single-state-workspaces) at the bottom of the chapter to identify which shape a workspace is.
