...
|---|---|---|
| `cluster_name` | `awsbnkctl-tf-cluster` | EKS cluster name. Mirrors `config.yaml`'s `cluster.name`. |
| `node_min_size` / `node_max_size` / `node_desired_size` | `2` / `10` / `3` | Self-managed node group sizing across the 3-AZ VPC. PRD 07 default: 3 × c5n.4xlarge. |
| `create_eks_cluster` | `true` | Set `false` to adopt an existing cluster. Pair with `eks_cluster_name`. |
| `cluster_version` | `"1.30"` | EKS Kubernetes minor. Quote it — YAML/HCL parses `1.30` as float otherwise. |
...
# perf-test.tfvars contains: node_desired_size = 8
...
- Static AWS keys never live in `terraform.tfvars` — they're resolved at apply time via the SDK default chain (env / profile / IRSA / IMDS). The supply-chain S3 bucket auth uses IRSA, not static HMAC keys. `terraform.tfstate` is still sensitive (carries resource IDs, IRSA role ARNs, KMS key references) — `chmod 0600`, never commit, treat the workspace as a secret store.
...
node_desired_size = 6
node_instance_types = ["c5n.9xlarge"]   # 36 vCPU / 96 GB per node
EOF
...
|---|---|
| Cluster identity, region, EKS Kubernetes version, node group size | `config.yaml` (via `awsbnkctl init` or by hand) |
