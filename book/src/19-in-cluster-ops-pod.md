...
> **Sprint 6 retarget — installer YAML now IRSA-shaped.** The embedded manifest at `internal/exec/k8s_install.yaml` was retargeted in Sprint 6 from the inherited IBM Trusted-Profile shape to the AWS IRSA shape. The pod lands as `awsbnkctl-ops` in the `awsbnkctl-ops` namespace; the ServiceAccount carries `eks.amazonaws.com/role-arn` so the EKS pod-identity webhook auto-injects `AWS_ROLE_ARN` + `AWS_WEB_IDENTITY_TOKEN_FILE` — no static-key Secret needed. The `--trusted-profile=auto` flow's AWS equivalent (`--irsa-role=auto`) is the v1.x roadmap; until it lands, the operator pre-creates the IRSA role per [chapter 25 §"IRSA trust chain"](./25-cos-supply-chain.md) and passes its ARN via `OPS_IRSA_ROLE_ARN`. The chapter below describes the IRSA-shaped design as Sprint 6 landed it. The body sections below were inherited and partially retargeted in Sprint 5/6 — any remaining references to `awsbnkctl-ibm-creds` / IBM Trusted Profiles are stale prose tracked for cleanup in [`docs/PLAN.md`](https://github.com/JLCode-tech/awsbnkctl/blob/main/docs/PLAN.md) §"What's deferred to post-v1.0".
