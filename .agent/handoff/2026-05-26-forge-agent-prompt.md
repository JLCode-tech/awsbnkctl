...
> (`syd-tracer`, BNK 2.3.0) is UP in AWS account `123456789012`, region `ap-southeast-2`,
...
- Operator / cluster-creator identity (implicit cluster-admin on syd-tracer):
  `arn:aws:sts::123456789012:assumed-role/AWSReservedSSO_Users_28497d15a8cf2cd3/J.Lucia@F5.com`
...
- The EKS clusters are `CONFIG_MAP` auth mode. forge's EKS-access-entry method requires the cluster to be
  ...
  is via the `aws-auth` ConfigMap; syd-tracer currently maps only `arn:aws:iam::123456789012:role/syd-tracer-eks-node-role`.
...
- Operator SSO permission-set role: `AWSReservedSSO_Users_28497d15a8cf2cd3`
  (full IAM ARN with SSO path: `arn:aws:iam::123456789012:role/aws-reserved/sso.amazonaws.com/AWSReservedSSO_Users_28497d15a8cf2cd3`).
- awsbnkctl created `syd-tracer` while assuming THIS exact SSO role (`AWS_PROFILE=Users-123456789012`), so it
...
1. **How forge obtains + refreshes the AWS SSO session** for the project (the creds expire; `aws sso login` mints them
   under the operator's profile `Users-123456789012`). Decide whether forge reads the operator's SSO cache or runs its
...
Observed gap: credential template id=1 ("AWS Production") IS sso-enabled for account 123456789012 / role
...
- For role `Users` on account 123456789012, region us-east-1, start_url `https://d-906774a4a9.awsapps.com/start/#`.
