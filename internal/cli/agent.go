...
import (
	...
	"github.com/JLCode-tech/awsbnkctl/internal/embedded"
)

// awsbnkctl's "agentic mode" (mirrors the bnkctl ecosystem standard): the binary
...
#   llm --system "$(cat AGENTS.md)" "Act as solution-architect; read cluster.yaml; confirm scope."
`, dir, base)
	},
...
var agentCmd = &cobra.Command{
	...
	Long: `Agentic mode. awsbnkctl embeds no LLM — bring your own coding-agent CLI.

  awsbnkctl agent init        Scaffold AGENTS.md + personas/ + journal/ into the workspace
  awsbnkctl agent             List supported CLIs + this workspace's default
  awsbnkctl agent <cli>       Print the invocation to launch <cli> against the workspace
...
var agentInitCmd = &cobra.Command{
	...
	Long: `Copies AGENTS.md, the persona role contracts, and a decisions.md seed
into the workspace dir, and creates an empty journal/. Safe to re-run:
existing files are left untouched (your edits survive).`,
	Args: cobra.NoArgs,
	RunE: runAgentInit,
}
...
func runAgent(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		...
		fmt.Fprintln(out, "\nRun:  awsbnkctl agent <name>   to print its invocation,")
		fmt.Fprintln(out, "      awsbnkctl agent init       to scaffold the persona files first.")
		return nil
	}
	...
	if _, err := os.Stat(filepath.Join(dir, "AGENTS.md")); err != nil {
		fmt.Fprintf(out, "\n# NOTE: %s has no AGENTS.md yet — run `awsbnkctl agent init` first.\n", dir)
	}
	return nil
}
...
func runAgentInit(cmd *cobra.Command, _ []string) error {
	...
	fmt.Fprintf(out, "  Next: awsbnkctl -w %s agent claude\n", resolvedWorkspaceName())
	return nil
}
