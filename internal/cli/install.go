...
var installCmd = &cobra.Command{
	Use:   "install",
	Short: "Copy the running awsbnkctl binary into a directory on PATH",
	Long: `Install the awsbnkctl binary you're currently running into a directory
...
func runInstall(_ *cobra.Command, _ []string) error {
	...
	if !isOnPATH(destDir) {
		...
		fmt.Fprintln(os.Stderr, "  (open a new shell or run `hash -r` if `awsbnkctl` doesn't resolve immediately)")
	}
	return nil
}
