...
// source), the per-awsbnkctl-workspace state directory, and a configured
...
func Open(
	...
	if wsCfg == nil {
		return nil, fmt.Errorf("workspace config is nil (run `awsbnkctl init`)")
	}
	...
	backendOverride := filepath.Join(sourceDir, "awsbnkctl_backend_override.tf")
	backendBody := fmt.Sprintf(`# Generated by awsbnkctl. Do not edit by hand.
...
// StateDir is the awsbnkctl per-workspace state root.
...
// config.yaml). Optional — if present, awsbnkctl passes it to terraform
...
// awsbnkctl's RenderTFVars doesn't expose (testing_*, roks_min_worker_*,
...
// (excluding api_key — see WriteTFVars in vars.go). awsbnkctl-managed
...
// name as a subdir; awsbnkctl pre-creates them in Open.
...
// override land cleanly even if a previous awsbnkctl run initialised
